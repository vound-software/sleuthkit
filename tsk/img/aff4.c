    ...
    size_t len)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr,
            "aff4_image_read: byte offset: %" PRIu64 " len: %" PRIuSIZE
            "\n", offset, len);

    if (offset > img_info->size) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_IMG_READ_OFF);
        tsk_error_set_errstr("aff4_image_read - %" PRIu64, offset);
        return -1;
    }
    ...
    if (cnt < 0) {
        ...
        tsk_error_set_errstr("aff4_image_read - offset: %" PRIu64
    ...
    const TSK_TCHAR * const a_images[], unsigned int a_ssize)
{
    ...
    wcstombs(filename, a_images[0], newsize);
#else
    filename = a_images[0];
#endif
