...
#define XWAYS_OEM_NAME "NTFS CTR"
 ...
 */
    typedef struct {
        ...
        uint8_t f1[2];          // 42 XP Only
        uint8_t entry[4];       // 44 XP Only - Number of this entry
    } ntfs_mft;
...
#define NTFS_XWAYS          NTFS_VINFO_XP
...
*/
    ...
    extern uint32_t xnt2unixtime(uint64_t ntdate);
    extern uint32_t xnt2nano(uint64_t ntdate);
    extern uint8_t xwfs2_attrname_lookup(TSK_FS_INFO *, uint16_t, char *,
        int);
    ...
    extern int xwfs2_name_cmp(TSK_FS_INFO *, const char *, const char *);

    extern uint8_t gwfs2_find_file(TSK_FS_INFO * fs, TSK_INUM_T inode_toid,
