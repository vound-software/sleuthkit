/*
** ntfs -stripped to handle xwfs2
** The Sleuth Kit
**
** Content and meta data layer support for the XWFS2 file system
**
**
 ...
 **********************************************************************/
  ...
  */
TSK_RETVAL_ENUM
xwfs2_dinode_lookup(NTFS_INFO* a_ntfs, char* a_buf, TSK_INUM_T a_mftnum)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr,
            "xwfs2_dinode_lookup: Processing MFT %" PRIuINUM "\n",
            a_mftnum);
    ...
    if (!a_ntfs->mft_data) {
        ...
        for (data_run = a_ntfs->mft_data->nrd.run;
            ...
            if ((offset < 0) || (data_run->len >= (TSK_DADDR_T)(LLONG_MAX / a_ntfs->csize_b))) {
                ...
                tsk_error_set_errstr
                ("xwfs2_dinode_lookup: Overflow when calculating run length");
                return TSK_COR;
            }
            ...
            if (offset < run_len) {
                ...
                if (tsk_verbose)
                    tsk_fprintf(stderr,
                        "xwfs2_dinode_lookup: Found in offset: %"
                ...
                if (run_len < offset + a_ntfs->mft_rsize_b) {
                    ...
                    if (tsk_verbose)
                        tsk_fprintf(stderr,
                            "xwfs2_dinode_lookup: Entry crosses run border\n");
                ...
                if (tsk_verbose)
                    tsk_fprintf(stderr,
                        "xwfs2_dinode_lookup: Entry address at: %"
                        PRIdOFF "\n", mftaddr_b);
                break;
            }
    ...
    if (mftaddr2_b) {
        ...
        if (cnt != (ssize_t)mftaddr_len) {
            ...
            ("xwfs2_dinode_lookup: Error reading MFT Entry (part 1) at %"
                PRIdOFF, mftaddr_b);
            return TSK_ERR;
        }
        ...
        if (cnt != (ssize_t)(a_ntfs->mft_rsize_b - mftaddr_len)) {
            ...
            tsk_error_set_errstr2
            ("xwfs2_dinode_lookup: Error reading MFT Entry (part 2) at %"
                PRIdOFF, mftaddr2_b);
            return TSK_ERR;
        }
    }
        ...
        if (cnt != a_ntfs->mft_rsize_b) {
            ...
            tsk_error_set_errstr2
            ("xwfs2_dinode_lookup: Error reading MFT Entry at %"
                PRIdOFF, mftaddr_b);
            return TSK_ERR;
        }
    }
...
xwfs2_make_data_run(NTFS_INFO* ntfs, TSK_OFF_T start_vcn,
    ...
    *a_data_run_head = NULL;
    ...
    while (NTFS_RUNL_LENSZ(run) != 0) {
        ...
        if (NTFS_RUNL_LENSZ(run) > 8) {
            ...
            tsk_error_set_errno(TSK_ERR_FS_INODE_COR);
            tsk_error_set_errstr
            ("xwfs2_make_run: Run length is too large to process");
            tsk_fs_attr_run_free(*a_data_run_head);
            *a_data_run_head = NULL;
            return TSK_COR;
        }
        for (i = 0, data_run->len = 0; i < NTFS_RUNL_LENSZ(run); i++) {
            ...
            if (tsk_verbose)
                tsk_fprintf(stderr,
                    "xwfs2_make_data_run: Len idx: %i cur: %"
        ...
        if (data_run->len > fs->block_count) {
            ...
            tsk_error_set_errno(TSK_ERR_FS_INODE_COR);
            tsk_error_set_errstr
            ("xwfs2_make_run: Run length is larger than file system");
            tsk_fs_attr_run_free(*a_data_run_head);
            *a_data_run_head = NULL;
            return TSK_COR;
        }
        ...
        for (i = 0, data_run->addr = 0; i < NTFS_RUNL_OFFSZ(run); i++) {
            ...
            if (tsk_verbose)
                tsk_fprintf(stderr,
                    "xwfs2_make_data_run: Off idx: %i cur: %"
                    PRIu8 " (%" PRIx8 ") tot: %" PRIuDADDR
                    " (%" PRIxDADDR ")\n", i,
        ...
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_make_data_run: Signed addr_offset: %"
        ...
        if (((addr_offset == -1) && (prev_addr == 0))
            ...
            data_run->addr = 0;
            if (tsk_verbose)
                tsk_fprintf(stderr, "xwfs2_make_data_run: Sparse Run\n");
        }
         ...
         */
         // @@@ BC: we'll need to pass in an inode value for this check
            ...
            if (data_run->addr + data_run->len > fs->block_count) {
                ...
                tsk_error_set_errno(TSK_ERR_FS_INODE_COR);
                tsk_error_set_errstr
                ("xwfs2_make_run: Run offset and length is larger than file system");
                tsk_fs_attr_run_free(*a_data_run_head);
                *a_data_run_head = NULL;
                return TSK_COR;
            }
 ...
 */
static void
xwfs2_uncompress_reset(NTFS_COMP_INFO* comp)
{
 ...
 */
static int
xwfs2_uncompress_setup(TSK_FS_INFO* fs, NTFS_COMP_INFO* comp,
    uint32_t compunit_size_c)
{
    ...
    xwfs2_uncompress_reset(comp);

    return 0;
}

static void
xwfs2_uncompress_done(NTFS_COMP_INFO* comp)
{
    free(comp->uncomp_buf);
    comp->uncomp_buf = NULL;
    free(comp->comp_buf);
 ...
 */
static uint8_t
xwfs2_uncompress_compunit(NTFS_COMP_INFO* comp)
{
    size_t cl_index;
    ...
    for (cl_index = 0; cl_index + 1 < comp->comp_len;) {
        ...
        if (blk_end > comp->comp_len) {
            ...
            tsk_error_set_errstr
            ("xwfs2_uncompress_compunit: Block length longer than buffer length: %"
                PRIuSIZE "", blk_end);
            return 1;
        }
        ...
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_uncompress_compunit: Block size is %" PRIuSIZE "\n",
                blk_size);
        ...
        if ((iscomp) && (blk_size - 2 != 4096)) {
            ...
            while (cl_index < blk_end) {
                ...
                if (tsk_verbose)
                    tsk_fprintf(stderr,
                        "xwfs2_uncompress_compunit: New Tag: %x\n", header);
                ...
                for (a = 0; a < 8 && cl_index < blk_end; a++) {
                    ...
                    if ((header & NTFS_TOKEN_MASK) == NTFS_SYMBOL_TOKEN) {
                        if (tsk_verbose)
                            tsk_fprintf(stderr,
                                "xwfs2_uncompress_compunit: Symbol Token: %"
                                PRIuSIZE "\n", cl_index);

                        if (comp->uncomp_idx >= comp->buf_size_b) {
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_uncompress_compunit: Trying to write past end of uncompression buffer: %"
                     ...
                     */
                        ...
                        if (cl_index + 1 >= blk_end) {
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_uncompress_compunit: Phrase token index is past end of block: %d",
                                a);
                            return 1;
                        }
                        ...
                        if (shift > 12) {
                            ...
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_uncompress_compunit: Shift is too large: %d", shift);
                            return 1;
                        }
                        ...
                        if (tsk_verbose)
                            tsk_fprintf(stderr,
                                "xwfs2_uncompress_compunit: Phrase Token: %"
                                PRIuSIZE "\t%d\t%d\t%x\n", cl_index,
                        ...
                        if (offset > comp->uncomp_idx) {
                            ...
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_uncompress_compunit: Phrase token offset is too large:  %d (max: %"
                            ...
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_uncompress_compunit: Phrase token length is too large:  %d (max: %" PRIuSIZE")",
                                length,
                            ...
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_uncompress_compunit: Phrase token length is too large for rest of uncomp buf:  %" PRIuSIZE" (max: %"
                                PRIuSIZE ")",
        ...
        // this block contains uncompressed data
            ...
            while (cl_index < blk_end && cl_index < comp->comp_len) {
                ...
                if (comp->uncomp_idx >= comp->buf_size_b) {
                    ...
                    tsk_error_set_errno(TSK_ERR_FS_FWALK);
                    tsk_error_set_errstr
                    ("xwfs2_uncompress_compunit: Trying to write past end of uncompression buffer (1) -- corrupt data?)");
                    return 1;
                }
 ...
 */
static uint8_t
xwfs2_proc_compunit(NTFS_INFO* ntfs, NTFS_COMP_INFO* comp,
    TSK_DADDR_T* comp_unit, uint32_t comp_unit_size)
{
    ...
    if (sparse) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_proc_compunit: Unit is fully sparse\n");
     ...
     * unit is compressed */
        ...
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_proc_compunit: Unit is compressed\n");
        ...
        for (a = 0; a < comp_unit_size; a++) {
            ...
            if (cnt != fs->block_size) {
                ...
                tsk_error_set_errstr2
                ("xwfs2_proc_compunit: Error reading block at %"
        ...
        if (xwfs2_uncompress_compunit(comp)) {
            return 1;
        }
    }
    ...
    /* Uncompressed data */
        ...
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_proc_compunit: Unit is not compressed\n");
 ...
 */
static uint8_t
xwfs2_attr_walk_special(const TSK_FS_ATTR* fs_attr,
    int flags, TSK_FS_FILE_WALK_CB a_action, void* ptr)
{
    ...
    if ((fs_attr == NULL) || (fs_attr->fs_file == NULL)
        ...
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr
        ("xwfs2_attr_walk_special: Null arguments given\n");
        return 1;
    }
    ...
    if (fs_attr->flags & TSK_FS_ATTR_COMP) {
        ...
        if (fs_attr->nrd.compsize <= 0) {
            tsk_error_set_errno(TSK_ERR_FS_FWALK);
            tsk_error_set_errstr
            ("xwfs2_attrwalk_special: Compressed attribute has compsize of 0 (%"
                PRIuINUM ")", fs_attr->fs_file->meta->addr);
            return 1;
        }

        /* Allocate the buffers and state structure */
        if (xwfs2_uncompress_setup(fs, &comp, fs_attr->nrd.compsize)) {
            return 1;
        }
            ...
            if (fs_attr_run->flags & TSK_FS_ATTR_RUN_FLAG_FILLER) {
                if (fs_attr_run->addr != 0) {
                    ...
                    if (fs_attr->fs_file->meta->
                        ...
                        tsk_error_set_errno(TSK_ERR_FS_GENFS);
                    tsk_error_set_errstr
                    ("xwfs2_attr_walk_special: Filler Entry exists in fs_attr_run %"
                    ...
                    if ((fs_attr_run->len > LLONG_MAX)
                        ...
                        if (fs_attr->fs_file->meta->
                            ...
                            tsk_error_set_errno(TSK_ERR_FS_GENFS);
                        tsk_error_set_errstr
                        ("xwfs2_attr_walk_special: Attribute run length is too large %"
                            PRIuDADDR "@%" PRIuDADDR " - type: %" PRIu32
                            "  id: %d Meta: %" PRIuINUM " Status: %s",
                            fs_attr_run->len, fs_attr_run->addr, fs_attr->type,
                            fs_attr->id, fs_attr->fs_file->meta->addr,
            ...
            for (len_idx = 0; len_idx < fs_attr_run->len; len_idx++) {
                ...
                if (addr > fs->last_block) {
                    ...
                    if (fs_attr->fs_file->meta->
                        ...
                        tsk_error_set_errno(TSK_ERR_FS_BLK_NUM);
                    tsk_error_set_errstr
                    ("xwfs2_attr_walk_special: Invalid address in run (too large): %"
                ...
                if ((comp_unit_idx == fs_attr->nrd.compsize)
                    ...
                    // decompress the unit
                    if (xwfs2_proc_compunit(ntfs, &comp, comp_unit,
                        comp_unit_idx)) {
                    ...
                    for (i = 0; i < comp_unit_idx; i++) {
                        ...
                        if (i * fs->block_size + read_len >
                            ...
                            tsk_error_set_errno(TSK_ERR_FS_FWALK);
                            tsk_error_set_errstr
                            ("xwfs2_attrwalk_special: Trying to read past end of uncompressed buffer: %"
        ...
        tsk_error_set_errno(TSK_ERR_FS_FWALK);
        tsk_error_set_errstr
        ("xwfs2_attrwalk_special: called with non-special attribute: %x",
            fs_attr->flags);
        return 1;
    }
}
 ...
 */
static ssize_t
xwfs2_file_read_special(const TSK_FS_ATTR* a_fs_attr,
    TSK_OFF_T a_offset, char* a_buf, size_t a_len)
{
    ...
    if ((a_fs_attr == NULL) || (a_fs_attr->fs_file == NULL)
        ...
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr
        ("xwfs2_file_read_special: NULL parameters passed");
        return -1;
    }
    ...
    if (a_fs_attr->flags & TSK_FS_ATTR_COMP) {
        ...
        if (a_fs_attr->nrd.compsize <= 0) {
            tsk_error_set_errno(TSK_ERR_FS_FWALK);
            tsk_error_set_errstr
            ("xwfs2_file_read_special: Compressed attribute has compsize of 0");
            return -1;
        }
        ...
        if (a_offset >= a_fs_attr->size) {
            ...
            tsk_error_set_errstr("xwfs2_file_read_special - %" PRIdOFF
        ...
        if (a_offset >= a_fs_attr->nrd.initsize) {
            ...
            if (tsk_verbose)
                fprintf(stderr,
                    "xwfs2_file_read_special: Returning 0s for read past end of initsize (%"
                    PRIuINUM ")\n", a_fs_attr->fs_file->meta->addr);
        ...
        /* Allocate the buffers and state structure */
        if (xwfs2_uncompress_setup(fs, &comp, a_fs_attr->nrd.compsize)) {
            return -1;
        }
        ...
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr
        ("xwfs2_file_read_special: called with non-special attribute: %x",
            a_fs_attr->flags);
        return -1;
    }
}
...
static TSK_RETVAL_ENUM xwfs2_proc_attrlist(NTFS_INFO*, TSK_FS_FILE*,
 ...
 */
static TSK_RETVAL_ENUM
xwfs2_proc_attrseq(NTFS_INFO* ntfs,
    ...
    TSK_INUM_T a_attrinum, const NTFS_ATTRLIST_MAP* a_attr_map, TSK_STACK* a_seen_inum_list)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr,
            "xwfs2_proc_attrseq: Processing extended entry for primary entry %"
            PRIuINUM "\n", fs_file->meta->addr);

    if (fs_file->meta->attr == NULL) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("Null attribute list in xwfs2_proc_attrseq");
        return TSK_ERR;
    }
    ...
    if (len > ntfs->mft_rsize_b) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("invalid length in xwfs2_proc_attrseq");
        return TSK_ERR;
    }
     ...
     * an xwfs2_attr, so make sure there is at least room for that */
    for (attr = a_attrseq; ((uintptr_t)attr >= (uintptr_t)a_attrseq)
        ...
        if (attr->nlen && nameoff + (uint32_t)attr->nlen * 2 < tsk_getu32(fs->endian, attr->len)) {
            ...
            if (retVal != TSKconversionOK) {
                if (tsk_verbose)
                    tsk_fprintf(stderr,
                        "xwfs2_proc_attrseq: Error converting NTFS attribute name to UTF8: %d %"
        ...
        if (attr->res == NTFS_MFT_RES) {
            ...
            if (tsk_verbose)
                tsk_fprintf(stderr,
                    "xwfs2_proc_attrseq: Resident Attribute in Type: %"
            ...
            if (((uintptr_t)attr + 24) > ((uintptr_t)a_attrseq + len)) {
                ...
                tsk_error_set_errstr("xwfs2_attr_walk: Resident attribute %"
                    PRIuINUM "-%" PRIu32
         ...
         */
            ...
            if (tsk_verbose)
                tsk_fprintf(stderr,
                    "xwfs2_proc_attrseq: Non-Resident Attribute Type: %"
                    PRIu32 " Id: %" PRIu16 " IdNew: %" PRIu16
            ...
            if (((uintptr_t)attr + 64) > ((uintptr_t)a_attrseq + len)) {
                tsk_error_reset();
                tsk_error_set_errno(TSK_ERR_FS_CORRUPT);
                tsk_error_set_errstr("xwfs2_attr_walk: Non-Resident attribute %"
                    PRIuINUM "-%" PRIu32
                    " starting offset and length too large",
                    fs_file->meta->addr, type);
                return TSK_COR;
            ...
            if (tsk_getu16(fs->endian, attr->c.nr.run_off) > tsk_getu32(fs->endian, attr->len)) {
                if (tsk_verbose)
                    tsk_fprintf(stderr, "xwfs2_proc_attrseq: run offset too big\n");
                break;
            }

            /* convert the run to generic form */
            retval = xwfs2_make_data_run(ntfs,
                ...
                for (i = 0; i < cnt; i++) {
                    ...
                    if (fs_attr2->type == type) {
                        if (((name[0] == '\0') && (fs_attr2->name == NULL))
                            ...
                            if (tsk_verbose)
                                tsk_fprintf(stderr,
                                    "xwfs2_proc_attrseq: Updating id from 0 to %"
                                    PRIu16 "\n", id_new);
                            break;
                        }
                    }
            ...
            if (tsk_getu16(fs->endian, attr->c.nr.compusize) > 16) {
                ...
                tsk_error_reset();
                tsk_error_set_errno(TSK_ERR_FS_CORRUPT);
                tsk_error_set_errstr("xwfs2_proc_attrseq: Compression unit size 2^%d too large",
            ...
            if (tsk_getu16(fs->endian, attr->c.nr.compusize) > 0) {
                ...
                if (data_flag & TSK_FS_ATTR_COMP) {
                    if (tsk_verbose)
                        fprintf(stderr,
                            "xwfs2_proc_attrseq: Clearing compression setting for attribute %"
            ...
            if (fs_attr == NULL) {
                ...
                if (fs_file->meta->flags & TSK_FS_META_FLAG_COMP) {
                    fs_attr->w = xwfs2_attr_walk_special;
                    fs_attr->r = xwfs2_file_read_special;
                }
        ...
        if (type == NTFS_ATYPE_SI) {
            ...
            fs_file->meta->mtime =
                xnt2unixtime(tsk_getu64(fs->endian, si->mtime));
            fs_file->meta->mtime_nano =
                xnt2nano(tsk_getu64(fs->endian, si->mtime));

            fs_file->meta->atime =
                xnt2unixtime(tsk_getu64(fs->endian, si->atime));
            fs_file->meta->atime_nano =
                xnt2nano(tsk_getu64(fs->endian, si->atime));

            fs_file->meta->ctime =
                xnt2unixtime(tsk_getu64(fs->endian, si->ctime));
            fs_file->meta->ctime_nano =
                xnt2nano(tsk_getu64(fs->endian, si->ctime));

            fs_file->meta->crtime =
                xnt2unixtime(tsk_getu64(fs->endian, si->crtime));
            fs_file->meta->crtime_nano =
                xnt2nano(tsk_getu64(fs->endian, si->crtime));
        ...
        /* File Name (always resident) */
            ...
            fs_file->meta->time2.ntfs.fn_mtime =
                xnt2unixtime(tsk_getu64(fs->endian, fname->mtime));
            fs_file->meta->time2.ntfs.fn_mtime_nano =
                xnt2nano(tsk_getu64(fs->endian, fname->mtime));

            fs_file->meta->time2.ntfs.fn_atime =
                xnt2unixtime(tsk_getu64(fs->endian, fname->atime));
            fs_file->meta->time2.ntfs.fn_atime_nano =
                xnt2nano(tsk_getu64(fs->endian, fname->atime));

            fs_file->meta->time2.ntfs.fn_ctime =
                xnt2unixtime(tsk_getu64(fs->endian, fname->ctime));
            fs_file->meta->time2.ntfs.fn_ctime_nano =
                xnt2nano(tsk_getu64(fs->endian, fname->ctime));

            fs_file->meta->time2.ntfs.fn_crtime =
                xnt2unixtime(tsk_getu64(fs->endian, fname->crtime));
            fs_file->meta->time2.ntfs.fn_crtime_nano =
                xnt2nano(tsk_getu64(fs->endian, fname->crtime));
    ...
    if (fs_attr_attrl) {
        ...
        if ((retval = xwfs2_proc_attrlist(ntfs, fs_file, fs_attr_attrl, a_seen_inum_list)) != TSK_OK) {
            return retval;
        }
    }
 ...
 */
static TSK_RETVAL_ENUM
xwfs2_proc_attrlist(NTFS_INFO* ntfs,
    TSK_FS_FILE* fs_file, const TSK_FS_ATTR* fs_attr_attrlist, TSK_STACK* processed_inum_list)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr,
            "xwfs2_proc_attrlist: Processing entry %"
            PRIuINUM "\n", fs_file->meta->addr);
    ...
    for (list = (ntfs_attrlist*)buf;
        ...
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_proc_attrlist: mft: %" PRIuINUM
    ...
    for (a = 0; a < mftToDoCnt; a++) {
        ...
        if ((retval =
            xwfs2_dinode_lookup(ntfs, (char*)mft,
                mftToDo[a])) != TSK_OK) {
        ...
        if (tsk_getu48(fs->endian, mft->base_ref) != fs_file->meta->addr) {
            ...
            if (((tsk_getu16(fs->endian,
                ...
                tsk_error_reset();
                tsk_error_set_errno(TSK_ERR_FS_INODE_COR);
                tsk_error_set_errstr("xwfs2_proc_attrlist: MFT %" PRIuINUM
        ...
        if (tsk_getu16(fs->endian, mft->attr_off) > ntfs->mft_rsize_b) {
            if (tsk_verbose)
                tsk_fprintf(stderr, "xwfs2_proc_attrlist: corrupt MFT entry attribute offsets\n");
            continue;
        }
        ...
        if ((retval =
            xwfs2_proc_attrseq(ntfs, fs_file, (ntfs_attr*)((uintptr_t)
 ...
 */
static TSK_RETVAL_ENUM
xwfs2_dinode_copy(NTFS_INFO* ntfs, TSK_FS_FILE* a_fs_file, char* a_buf,
    TSK_INUM_T a_mnum)
{
    ntfs_attr* attr;
    TSK_FS_INFO* fs = (TSK_FS_INFO*)&ntfs->fs_info;
    ...
    if ((a_fs_file == NULL) || (a_fs_file->meta == NULL)) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("xwfs2_dinode_copy: NULL fs_file given");
        return TSK_ERR;
    }
    ...
    if (tsk_getu16(fs->endian, mft->attr_off) > ntfs->mft_rsize_b) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("xwfs2_dinode_copy: corrupt MFT entry attribute offsets");
        return TSK_ERR;
    }
    ...
    attr =
        (ntfs_attr*)((uintptr_t)mft + tsk_getu16(fs->endian,
            mft->attr_off));
    if ((retval = xwfs2_proc_attrseq(ntfs, a_fs_file, attr,
        ntfs->mft_rsize_b - tsk_getu16(fs->endian,
 ...
 */
static uint8_t
xwfs2_load_attrs(TSK_FS_FILE* a_fs_file)
{
    if ((a_fs_file == NULL) || (a_fs_file->meta == NULL)) {
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("xwfs2_load_attrs: called with NULL pointers");
        return 1;
    }
    ...
    if (a_fs_file->meta->attr == NULL) {
        if (a_fs_file->meta->flags & TSK_FS_META_FLAG_UNALLOC)
            ...
            tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("xwfs2_load_attrs: attributes are NULL");
        return 1;
    }
    return 0;
}
 ...
 */
static uint8_t
xwfs2_inode_lookup(TSK_FS_INFO* fs, TSK_FS_FILE* a_fs_file,
    TSK_INUM_T mftnum)
{
    ...
    if (a_fs_file == NULL) {
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("xwfs2_inode_lookup: fs_file is NULL");
        return 1;
    }
    ...
    if (xwfs2_dinode_lookup(ntfs, mft, mftnum) != TSK_OK) {
        free(mft);
        return 1;
    }

    /* Copy the structure in ntfs to generic a_fs_file->meta */
    if (xwfs2_dinode_copy(ntfs, a_fs_file, mft, mftnum) != TSK_OK) {
        free(mft);
        return 1;
    }
 ...
 **********************************************************************/
  ...
  */
static uint8_t
xwfs2_load_attrdef(NTFS_INFO* ntfs)
{
    TSK_FS_FILE* fs_file;
    const TSK_FS_ATTR* fs_attr;
 ...
 */
uint8_t
xwfs2_attrname_lookup(TSK_FS_INFO* fs, uint16_t type, char* name, int len)
{
    NTFS_INFO* ntfs = (NTFS_INFO*)fs;
    ntfs_attrdef* attrdef;
    if (!ntfs->attrdef) {
        if (xwfs2_load_attrdef(ntfs))
            return 1;
    }
 ...
 * */
static uint8_t
xwfs2_load_bmap(NTFS_INFO* ntfs)
{
    ssize_t cnt = 0;
    ntfs_attr* attr = NULL;
    ...
    if (xwfs2_dinode_lookup(ntfs, (char*)mft, NTFS_MFT_BMAP) != TSK_OK) {
        goto on_error;
    }
    ...
    if (ntfs->bmap->addr > fs->last_block) {
        ...
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr
        ("xwfs2_load_bmap: Bitmap too large for image size: %" PRIuDADDR
            "", ntfs->bmap->addr);
        goto on_error;
    }
    cnt =
    ...
    if (cnt != fs->block_size) {
        ...
        tsk_error_set_errstr2("xwfs2_load_bmap: Error reading block at %"
...
xwfs2_load_ver(NTFS_INFO* ntfs)
{
    ...
    if (!fs_attr) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_INODE_COR);
        tsk_error_set_errstr("Volume Info attribute not found in $Volume, assimong XWays clone ");
        tsk_fs_file_close(fs_file);
        ntfs->ver = NTFS_XWAYS;
        goto end;
    }
 ...
 */
static uint8_t
xwfs2_sds_to_str(TSK_FS_INFO* a_fs, const ntfs_attr_sds* a_sds,
    char** a_sidstr)
{
    ntfs_sid* sid = NULL;
    ...
    *a_sidstr = NULL;
    ...
    if (((uintptr_t)&a_sds->self_rel_sec_desc + owner_offset) >
        ...
        tsk_error_set_errno(TSK_ERR_FS_INODE_COR);
        tsk_error_set_errstr
        ("xwfs2_sds_to_str: owner offset larger than a_sds length");
        return 1;
    }
    ...
    if (sid->revision == 1) {
        ...
        *a_sidstr = sid_str;
        ...
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr("xwfs2_sds_to_str: Invalid SID revision (%d)",
            sid->revision);
...
xwfs2_get_sds(TSK_FS_INFO* fs, uint32_t secid)
{
    uint32_t i = 0;
    NTFS_INFO* ntfs = (NTFS_INFO*)fs;
    ...
    if (sii == NULL) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr("xwfs2_get_sds: SII entry not found (%" PRIu32
            ")", secid);
        return NULL;
    }
    ...
    if ((uint32_t)sii_sds_file_off > ntfs->sds_data.size) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr("xwfs2_get_sds: SII offset too large (%" PRIu64
            ")", sii_sds_file_off);
        return NULL;
    }
        ...
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr("xwfs2_get_sds: SII entry size is invalid (%"
    ...
    if ((sds_secid == sii_secid) &&
        ...
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_get_sds: entry found was for wrong Security ID (%"
                PRIu32 " vs %" PRIu32 ")\n", sds_secid, sii_secid);
    ...
    tsk_error_reset();
    tsk_error_set_errno(TSK_ERR_FS_GENFS);
    tsk_error_set_errstr("xwfs2_get_sds: Got to end w/out data");
    return NULL;
}
#endif
 ...
 */
static uint8_t
xwfs2_file_get_sidstr(TSK_FS_FILE* a_fs_file, char** sid_str)
{
#if TSK_USE_SID
    ...
    *sid_str = NULL;
    ...
    if (!a_fs_file->meta->attr) {
        ...
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr
        ("xwfs2_file_get_sidstr: file argument has no meta data");
        return 1;
    }
    ...
    if (!fs_data) {
        tsk_error_set_errstr2("- xwfs2_file_get_sidstr:SI attribute");
        return 1;
    }
    ...
    if (!si) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_GENFS);
        tsk_error_set_errstr("xwfs2_file_get_sidstr: SI buf is NULL");
        return 1;
    }
    ...
    sds =
        xwfs2_get_sds(a_fs_file->fs_info,
    ...
    if (xwfs2_sds_to_str(a_fs_file->fs_info, sds, sid_str)) {
        tsk_release_lock(&ntfs->sid_lock);
        tsk_error_set_errstr2("- xwfs2_file_get_sidstr:SI attribute");
        return 1;
    }
 ...
 */
static void
xwfs2_proc_sii(TSK_FS_INFO* fs, NTFS_SXX_BUFFER* sii_buffer)
{
    ...
    for (sii_buffer_offset = 0; sii_buffer_offset < sii_buffer->size;
        ...
        if (tsk_getu32(fs->endian, idxrec->list.begin_off) > ntfs->idx_rsize_b) {
            if (tsk_verbose)
                tsk_fprintf(stderr, "xwfs2_proc_sii: corrupt offset\n");
            break;
        }
        ...
        while ((uintptr_t)sii + sizeof(ntfs_attr_sii) <= idx_buffer_end) {
            ...
            */
            ...
            if ((ntfs->sii_data.used + 1) * sizeof(ntfs_attr_sii) > ntfs->sii_data.size) {
                if (tsk_verbose)
                    tsk_fprintf(stderr, "xwfs2_proc_sii: data buffer too small\n");
 ...
 *
 * Note: This routine is called only from xwfs2_open and therefore does
 * not need to lock ntfs->sid_lock.
 *
 * @returns 1 on error (which occurs only if malloc or other system error).
 */
static uint8_t
xwfs2_load_secure(NTFS_INFO* ntfs)
{
    TSK_FS_INFO* fs = (TSK_FS_INFO*)&ntfs->fs_info;
    ...
    if (!secure) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: error opening $Secure file: %s\n",
    ...
    if (!fs_meta) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: $Secure file has no attributes\n");
        tsk_error_reset();
    ...
    if (!fs_attr_sii) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: error getting $Secure:$SII IDX_ALLOC attribute\n");
        tsk_error_reset();
        tsk_fs_file_close(secure);
        return 0;
    ...
    if (sii_buffer.size > 64000000) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: sii_buffer.size is too large: %z\n",
                sii_buffer.size);
        return 0;
    }
    ...
    if (cnt != (ssize_t)sii_buffer.size) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: error reading $Secure:$SII attribute: %s\n",
                tsk_error_get_errstr());
        tsk_error_reset();
    ...
    if (ntfs->sds_data.size > 64000000) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: ntfs->sds_data.size is too large: %z\n",
                ntfs->sds_data.size);
    ...
    if (cnt != (ssize_t)ntfs->sds_data.size) {
        if (tsk_verbose)
            tsk_fprintf(stderr,
                "xwfs2_load_secure: error reading $Secure:$SDS attribute: %s\n",
                tsk_error_get_errstr());
        tsk_error_reset();
...
xwfs2_block_getflags(TSK_FS_INFO* a_fs, TSK_DADDR_T a_addr)
{
    ...
    void* a_ptr)
{
    ...
    for (addr = a_start_blk; addr <= a_end_blk; addr++) {
        ...
        if (tsk_fs_block_get_flag(fs, fs_block, addr,
            ...
            tsk_error_set_errstr2
            ("xwfs2_block_walk: Error reading block at %" PRIuDADDR,
                addr);
            tsk_fs_block_free(fs_block);
            return 1;
        }
 ...
 */
static uint8_t
xwfs2_inode_walk(TSK_FS_INFO* fs, TSK_INUM_T start_inum,
    ...
    TSK_FS_META_WALK_CB a_action, void* ptr)
{
    ...
    if ((flags & TSK_FS_META_FLAG_ORPHAN)) {
        if (tsk_fs_dir_load_inum_named(fs) != TSK_OK) {
            tsk_error_errstr2_concat
            ("- xwfs2_inode_walk: identifying inodes allocated by file names");
            return 1;
        }
    }
    ...
    for (mftnum = start_inum; mftnum <= end_inum_tmp; mftnum++) {
        ...
        if ((retval =
            xwfs2_dinode_copy(ntfs, fs_file, (char*)mft,
                mftnum)) != TSK_OK) {
...
static uint8_t
xwfs2_fscheck(TSK_FS_INFO* fs, FILE* hFile)
{
    tsk_error_reset();
    tsk_error_set_errno(TSK_ERR_FS_UNSUPFUNC);
 ...
 */
static uint8_t
xwfs2_fsstat(TSK_FS_INFO* fs, FILE* hFile)
{
    NTFS_INFO* ntfs = (NTFS_INFO*)fs;
    TSK_FS_FILE* fs_file;
    const TSK_FS_ATTR* fs_attr;
 ...
 */
static uint8_t
xwfs2_istat(TSK_FS_INFO* fs, TSK_FS_ISTAT_FLAG_ENUM istat_flags, FILE* hFile,
    TSK_INUM_T inum, TSK_DADDR_T numblock, int32_t sec_skew)
{
    ...
    if (xwfs2_dinode_lookup(ntfs, (char*)mft, inum)) {
        free(mft);
        return 1;
    }
...
#if TSK_USE_SID
        xwfs2_file_get_sidstr(fs_file, &sid_str);
    ...
    for (idx = 0; idx < tsk_fs_attrlist_get_len(fs_file->meta->attr); idx++) {
        ...
        if (retVal != TSKconversionOK) {
            if (tsk_verbose)
                tsk_fprintf(stderr,
                    "xwfs2_istat: Error converting NTFS name in $FNAME to UTF8: %d",
                    retVal);
            *name8 = '\0';
        }
        /* Make sure it is NULL Terminated */
        ...
        if (sec_skew != 0) {
            ...
            tsk_fprintf(hFile, "Created:\t%s\n",
                tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->crtime)) - sec_skew, xnt2nano(tsk_getu64(fs->endian, fname->crtime)), timeBuf));
            tsk_fprintf(hFile, "File Modified:\t%s\n",
                tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->mtime)) - sec_skew, xnt2nano(tsk_getu64(fs->endian, fname->mtime)), timeBuf));
            tsk_fprintf(hFile, "MFT Modified:\t%s\n",
                tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->ctime)) - sec_skew, xnt2nano(tsk_getu64(fs->endian, fname->ctime)), timeBuf));
            tsk_fprintf(hFile, "Accessed:\t%s\n",
                tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->atime)) - sec_skew, xnt2nano(tsk_getu64(fs->endian, fname->atime)), timeBuf));

            tsk_fprintf(hFile, "\nOriginal times:\n");
        }

        tsk_fprintf(hFile, "Created:\t%s\n",
            tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->crtime)), xnt2nano(tsk_getu64(fs->endian, fname->crtime)), timeBuf));
        tsk_fprintf(hFile, "File Modified:\t%s\n",
            tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->mtime)), xnt2nano(tsk_getu64(fs->endian, fname->mtime)), timeBuf));
        tsk_fprintf(hFile, "MFT Modified:\t%s\n",
            tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->ctime)), xnt2nano(tsk_getu64(fs->endian, fname->ctime)), timeBuf));
        tsk_fprintf(hFile, "Accessed:\t%s\n",
            tsk_fs_time_to_str_subsecs(xnt2unixtime(tsk_getu64(fs->endian, fname->atime)), xnt2nano(tsk_getu64(fs->endian, fname->atime)), timeBuf));
    }
    ...
    if (fs_file->meta->attr) {
        ...
        for (i = 0; i < cnt; i++) {
            ...
            if (xwfs2_attrname_lookup(fs, fs_attr->type, type, 512)) {
...
static uint8_t
xwfs2_jopen(TSK_FS_INFO* fs, TSK_INUM_T inum)
{
    tsk_error_reset();
    tsk_error_set_errno(TSK_ERR_FS_UNSUPFUNC);
...
static uint8_t
xwfs2_jentry_walk(TSK_FS_INFO* fs, int flags,
...
static uint8_t
xwfs2_jblk_walk(TSK_FS_INFO* fs, TSK_DADDR_T start,
...
xwfs2_get_default_attr_type(const TSK_FS_FILE* a_file)
{
 ...
 */
TSK_FS_INFO*
xwfs2_open(TSK_IMG_INFO* img_info, TSK_OFF_T offset,
    TSK_FS_TYPE_ENUM ftype, const char * a_pass, uint8_t test)
{
    char* myname = "xwfs2_open";
    NTFS_INFO* ntfs = NULL;
    TSK_FS_INFO* fs = NULL;
    unsigned int len = 0;
    ssize_t cnt = 0;
    ...
    if (TSK_FS_TYPE_ISXWFS2(ftype) == 0) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("Invalid FS type in xwfs2_open");
        return NULL;
    }
    ...
    if (img_info->sector_size == 0) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_ARG);
        tsk_error_set_errstr("xwfs2_open: sector size is 0");
        return NULL;
    }
    ...
    if (_strnicmp(ntfs->fs->oemname, XWAYS_OEM_NAME, strlen(XWAYS_OEM_NAME)) != 0) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_UNSUPTYPE);
        tsk_error_set_errstr("Not a xwfs2 file system (oem name)");
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: Incorrect xwfs2 oem name\n");
        goto on_error;
    ...
    if (tsk_fs_guessu16(fs, ntfs->fs->magic, NTFS_FS_MAGIC)) {
        ...
        tsk_error_set_errstr("Not a supported file system (magic)");
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: Incorrect xwfs2 magic\n");
        goto on_error;
    }
    ...
    if ((ntfs->ssize_b == 0) || (ntfs->ssize_b % 512)) {
        ...
        tsk_error_set_errno(TSK_ERR_FS_MAGIC);
        tsk_error_set_errstr
        ("Not a xwfs2 file system (invalid sector size %d))",
            ntfs->ssize_b);
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: invalid sector size: %d\n",
                ntfs->ssize_b);
        goto on_error;
    }
    ...
    if ((ntfs->fs->csize != 0x01) &&
        ...
        tsk_error_set_errno(TSK_ERR_FS_MAGIC);
        tsk_error_set_errstr
        ("Not a xwfs2 file system (invalid cluster size %d)",
            ntfs->fs->csize);
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: invalid cluster size: %d\n",
                ntfs->fs->csize);
        goto on_error;
    }
    ...
    if (fs->block_count == 0) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_FS_MAGIC);
        tsk_error_set_errstr("Not a xwfs2 file system (volume size is 0)");
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: invalid volume size: 0\n");
        goto on_error;
    }
    ...
    if ((ntfs->mft_rsize_b == 0) || (ntfs->mft_rsize_b % 512)) {
        ...
        tsk_error_set_errno(TSK_ERR_FS_MAGIC);
        tsk_error_set_errstr
        ("Not a xwfs2 file system (invalid MFT entry size)");
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: invalid MFT entry size\n");
        goto on_error;
    }
    ...
    if ((ntfs->idx_rsize_b == 0) || (ntfs->idx_rsize_b % 512)) {
        ...
        tsk_error_set_errno(TSK_ERR_FS_MAGIC);
        tsk_error_set_errstr
        ("Not a xwfs2 file system (invalid idx record size %d)",
            ntfs->idx_rsize_b);
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: invalid idx record size %d\n",
                ntfs->idx_rsize_b);
        goto on_error;
    }
    ...
    if (tsk_getu64(fs->endian, ntfs->fs->mft_clust) > fs->last_block) {
        ...
        tsk_error_set_errno(TSK_ERR_FS_MAGIC);
        tsk_error_set_errstr
        ("Not a xwfs2 file system (invalid starting MFT clust)");
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: invalid starting MFT cluster\n");
        goto on_error;
    }
     ...
     */
    fs->inode_walk = xwfs2_inode_walk;
    fs->block_walk = xwfs2_block_walk;
    fs->block_getflags = xwfs2_block_getflags;

    fs->get_default_attr_type = xwfs2_get_default_attr_type;
    fs->load_attrs = xwfs2_load_attrs;

    fs->file_add_meta = xwfs2_inode_lookup;
    fs->dir_open_meta = xwfs2_dir_open_meta;
    fs->fsstat = xwfs2_fsstat;
    fs->fscheck = xwfs2_fscheck;
    fs->istat = xwfs2_istat;
    fs->close = xwfs2_close;
    fs->name_cmp = xwfs2_name_cmp;

    fs->fread_owner_sid = xwfs2_file_get_sidstr;
    fs->jblk_walk = xwfs2_jblk_walk;
    fs->jentry_walk = xwfs2_jentry_walk;
    fs->jopen = xwfs2_jopen;
    fs->journ_inum = 0;
    ...
    if ((ntfs->mft_file =
        ...
        if (tsk_verbose)
            fprintf(stderr,
                "xwfs2_open: Error opening $MFT (%s)\n", tsk_error_get());
        goto on_error;
    }
    ...
    if (!ntfs->mft_data) {
        ...
        if (tsk_verbose)
            fprintf(stderr,
                "xwfs2_open: Data attribute not found in $MFT (%s)\n",
                tsk_error_get());
        goto on_error;
    }
    ...
    if (xwfs2_load_ver(ntfs)) {
        if (tsk_verbose)
            fprintf(stderr,
                "xwfs2_open: Error loading file system version ((%s)\n",
                tsk_error_get());
        goto on_error;
    }

    /* load the data block bitmap data run into xwfs2_info */
    //There are not frre and unused bitmaps ? TODO - check 
    if (xwfs2_load_bmap(ntfs)) {
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: Error loading block bitmap (%s)\n",
                tsk_error_get());
        //goto on_error;
    }

    /* load the SID data into xwfs2_info ($Secure - $SDS, $SDH, $SII */


#if TSK_USE_SID
    if (xwfs2_load_secure(ntfs)) {
        if (tsk_verbose)
            fprintf(stderr, "xwfs2_open: Error loading Secure Info (%s)\n",
                tsk_error_get());
        goto on_error;
    }
#endif
